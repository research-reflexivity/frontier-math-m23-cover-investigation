...
\title[The Galois-fixed point on an $M_{23}$ Hurwitz scheme]
  {The Galois-fixed point on an $M_{23}$ Hurwitz scheme:\\
   pointed characteristic-$23$ reduction and exact reconstruction}
...
\hypersetup{
  pdftitle={The Galois-fixed point on an M23 Hurwitz scheme: pointed characteristic-23 reduction and exact reconstruction},
  pdfauthor={François Le Lay},
  pdfsubject={An exact arithmetic reconstruction and characteristic-23 explanation of the Galois-fixed point on an M23 Hurwitz scheme}
}
...
point, pointed stable reduction, nearby cycles, Fano plane, inverse Galois
problem}
...
$\epsilon(\Theta)$ with the same two geometric values.  Pointed stable
reduction, the odd fixed-point identity joining the Fano heptad to the
affine eight-point torsor, and specialization of a Lefschetz--Verdier pairing
on an auxiliary Ferrand pushout give the intrinsic comparison
\[
 \epsilon(\Theta)=e_{\mathrm{sing}}.
\]
This comparison does not use the identification of the seven exact maps with
the seven complex Nielsen classes.
...
supply the permutation local systems used in the pointed comparison, but
...
idempotent $e_{\mathrm{sing}}$.  The central result identifies them without
using the complex branch-cycle labeling for the comparison.

\begin{theorem}[Pointed characteristic-$23$ explanation of the fixed point]
\label{thm:connector}
...
\[
 \epsilon(\Theta)=e_{\mathrm{sing}}
\]
in the coordinate algebra of $\mathcal H_{\mathrm{in}}$.  The comparison is
obtained from the pointed characteristic-$23$ stable maps and does not use
the certified complex branch cycles of the seven reconstructed maps.
\end{theorem}

The proof uses a feature visible inside the distinguished cover.  Let
...
identity supplies the finite permutation local systems in the
normalization--conductor argument used to prove \cref{thm:connector}.
...
distinguished cover.  The second set contributes geometry to the comparison,
but it is not itself the moduli scheme and does not by itself distinguish the
fixed point.
...
\[
 ...
 \longrightarrow
 \text{pointed reduction at }23
 \longrightarrow
 \epsilon(\Theta)=e_{\mathrm{sing}}.
\]
\Cref{sec:mechanism} gives a short guide to this mechanism before the
computational details.  The full Hurwitz reconstruction and the proof of the
comparison come first.  The minimal projection and branch-fibre arithmetic
are then recorded as supporting results on the distinguished cover.  The
specialization theorem comes last as an independent application, so it does
not interrupt the fixed-point argument.
...
and the pointed characteristic-$23$ comparison that explains why the
finite-group and geometric invariants cut out the same component.  Supporting
...
The comparison theorem concerns the two primitive idempotents of
\[
 A_{\mathrm H}=K_0\times L.
\]
Write $e_1=(1,0)$ for the idempotent cutting out the degree-one
open-and-closed subscheme and $e_6=(0,1)$ for its degree-six complement.
Explaining the fixed point means constructing one of these idempotents from
intrinsic geometry and identifying it with an invariant already visible in
the Nielsen-class problem.  Merely relisting the seven branch-cycle triples
would identify the same point, but would not provide such an explanation.

On the finite-group side, work in the fixed degree-$23$ permutation
representation of $M_{23}$.  The two wild endpoint normalizers of a
normalized Nielsen triple give relative transporter elements in the
centralizer of the finite inertia involution.  Their images form an element
\[
 \Theta\in\FF_2[2^3{:}\PSL_3(2)].
\]
Its augmentation is $0$ on the distinguished point and $1$ on each of the
other six points.  Hence, once the exact $1+6$ arithmetic decomposition is
known, $\epsilon(\Theta)$ is the idempotent $e_6$.  This is a finite
calculation, but that calculation alone does not explain why the same
Boolean function should arise from reduction at $23$.

On the geometric side, let $\mathcal H_R$ be the normalization of
$\operatorname{Spec}\mathcal O_{K_0,\mathfrak q}$ in $A_{\mathrm H}$, where
$\mathfrak q$ is the prime above $23$.  Its closed fibre has three closed
points: a rational $E_8$ point, an unramified quadratic $E_8$ point, and a
ramified $A_2+A_6$ point.  Normalize each reduced source using the uniquely
marked zero and pole of the degree-$23$ map.  Evaluation of the resulting
coordinate $u$ at the distinguished singular point satisfies
\[
 (u-16)(u^2+1)(u^2+u+1)=0.
\]
The polynomial $e_{\mathrm{sing}}(u)$ taking the values $(0,1,1)$ on these
three factors lifts uniquely to $e_6$.  Unlike a label imported from complex
continuation, this singular position is defined directly on the pointed
characteristic-$23$ models.

The substantive issue is to compare these two constructions without first
identifying both of them with $e_6$ through the correspondence between the
exact maps and the complex Nielsen classes.  After a finite splitting
extension, the proof chooses one
labeling of the geometric degree-$23$ fibre and uses it for every component
of the pointed stable map.  The seven fixed sheets of the finite involution
form a Fano set $H_x$, while its eight two-sheet orbits form an affine torsor
$A_x$.  For every $c$ in the inertia centralizer,
\[
 |\Fix_{H_x}(c)|+|\Fix_{A_x}(c)|\equiv1\pmod2.
\]
This identity identifies the generic transporter augmentation with the
degree modulo $2$ of a finite intersection of correspondences.

At the closed fibre, the normalization keeps the labels separate, whereas
the conductor subscheme identifies labels having the same image under a
finite quotient map.  This is realized by a Ferrand pushout.  The
normalization--conductor distinguished triangle, after applying nearby
cycles, compares the diagonal pairing on the normalization with the pairing
obtained by pushforward along that quotient map.  The gluing element at the
marked wild node defines a character $\chi_{\mathrm{gl}}(n)\in\FF_2$; its
values are $0$ at the rational $E_8$ point and $1$ at the two points on the
sextic component.  Compatibility of the Lefschetz--Verdier pairing with
specialization gives
\[
 \epsilon(\Theta)
 =\operatorname{Tr}_{\mathrm{generic}}
 =\operatorname{Tr}_{\mathrm{closed}}
 =\chi_{\mathrm{gl}}(n)
 =e_{\mathrm{sing}}.
\]

The exact branch cycles remain essential for proving that
$\operatorname{Spec}(K_0\times L)$ is the full Hurwitz scheme.  They are not
used in the middle equality above.  This separation is the point of the
argument: exact reconstruction determines the finite \'etale Hurwitz scheme,
while pointed reduction explains its distinguished component.

The next section gives the full reconstruction and the detailed comparison
proof.  The two sections after it record explicit arithmetic on the known
cover and show why the unpointed Fano plane, affine cube, reflection, and
ruling field are suggestive but insufficient by themselves.

\section{Exact Hurwitz arithmetic and the fixed-point comparison}
\label{sec:hurwitz-scheme}

\subsection{Coordinate algebra and canonical models}

We now turn to the seven-cover moduli problem itself.  The section has three tasks: reconstruct
and identify the finite Hurwitz scheme, construct its component idempotents
independently from finite-group and characteristic-$23$ data, and prove their
intrinsic equality by pointed stable reduction.  The normalized canonical
...
both sides are $e_6$.  The intrinsic proof in \S\ref{sec:connector} instead
constructs the relative-transporter augmentation from the stable geometry.
Resolving only the coarse ADE source would be insufficient: the degree-one
and unramified quadratic reductions both have geometric type $E_8$.  The
proof therefore retains the arithmetic singular position together with the
incidence and gluing in a common labeling of the degree-$23$ geometric
fibre.
...
Hurwitz scheme.  We now give the promised intrinsic comparison with the
characteristic-$23$ model.

\subsection{Comparison by pointed characteristic-$23$ reduction}
\label{sec:connector}

Fix the natural degree-$23$ permutation representation
$M_{23}\subset S_{23}$.  By a \emph{labeling of the geometric fibre} we
mean an identification of its sheets with $\{1,\ldots,23\}$.  Changing the
labeling simultaneously conjugates every permutation in $S_{23}$, and the
labelings form an $S_{23}$-torsor.  The constructions below are first made
after a finite extension that trivializes this torsor and are then shown to
be invariant under simultaneous relabeling.

Let $x\in2A(\M)$, let $H_x=\Fix(x)$, and let $A_x$ be the set of its eight
two-element sheet orbits.  Thus $H_x$ is the seven-point Fano set and $A_x$
is an affine torsor under $\FF_2^3$.  Put
\[
 B_x=H_x\amalg A_x.
\]
...
After passing to the splitting extension just described, let
$D_y,D_z\simeq23{:}11$ be the two wild endpoint normalizers in the same
labeling.  For $D=D_y$ or $D_z$, define the finite correspondence
\[
\mathcal L_D=\{(b,x^d,b^d):b\in B_x,\ d\in D\}.
\]
The action of $D$ on $x^D$ is free, since the stabilizer order divides both
$253$ and $|\Cx|=2688$.  If $w\in x^{D_y}\cap x^{D_z}$, let $d_y(w)$ and
$d_z(w)$ be the unique transporter elements carrying $x$ to $w$.  Then
\[
 c_w=d_y(w)d_z(w)^{-1}\in\Cx,
\]
and equality of the two lifted orbit labels over $w$ is equivalent to
$b^{c_w}=b$.  Therefore \cref{lem:fano-affine-odd} gives
\begin{equation}
 \deg(\mathcal L_{D_y}\cap\mathcal L_{D_z})\bmod2
 =|x^{D_y}\cap x^{D_z}|\bmod2
 =\epsilon(\Theta).                                  \label{eq:connector-generic}
\end{equation}
This realizes the generic augmentation as the degree modulo $2$ of a finite
intersection; the relative transporter is used only to prove that degree is
odd.

We next record the normalizer-coset class of the gluing automorphism in each
of the three local characteristic-$23$ models.  Put $D=23{:}11$ and identify
\[
 N_{S_{23}}(D)/D=(23{:}22)/(23{:}11)\simeq C_2.
\]
Let
$\chi_{\mathrm{gl}}:N_{S_{23}}(D)/D\xrightarrow{\sim}\FF_2$
be the resulting character.  For a transition element $n$ at the marked
wild node, $\chi_{\mathrm{gl}}(n)$ records its normalizer coset.

\begin{computation}[Normalizer classes of the pointed stable reductions]
\label{comp:pointed-stable-returns}
After finite base change, the complete local traits at the three closed
points carry stable marked $\M$-models compatible with a single labeling of
the degree-$23$ geometric fibre.  If $n_P$ denotes the transition element at
the marked wild node, then
\begin{equation}
\begin{array}{c|ccc}
\text{closed point}&\text{rational }E_8&
 E_8,\ (e,f)=(1,2)&A_2+A_6,\ (e,f)=(2,2)\\
\hline
\chi_{\mathrm{gl}}(n_P)&0&1&1.
\end{array}
\label{eq:pointed-return-bits}
\end{equation}
Thus $\chi_{\mathrm{gl}}(n_P)$ has the same three values as
$e_{\mathrm{sing}}$.
\end{computation}

\begin{proof}
Stable reduction for the marked Galois covers extends the deck action and
the three branch sections after finite base change.  In Wewers's patching
datum, one group isomorphism $\lambda$ is shared by all tails; changing the
datum transports $\lambda$ and every tail map simultaneously
\cite[Section~4.2 and Theorem~4.5]{Wewers}.  Hence the two wild groups and
the finite inertia are expressed in one labeling up to simultaneous
conjugacy.

The exact normalized Newton charts compute the square class of the gluing
scalar: it is a square on the rational $E_8$ branch and a nonsquare on the
unramified $E_8$ branch.  On the ramified branch, the $A_2$ pole gives the
trivial class, while the formal annulus corresponding to the unique $A_6$
node has conductor pair $(11,15)$ and gives the nonsquare class.  For a
pointed $23{:}11$ node with lower conductor $h=15$, scaling the natural
degree-$23$ quotient parameter by $\eta$ scales the additive wild line by
$\lambda^{-h}$, where $\lambda^{11}=\eta$.  Since $11$ and $15$ are odd,
the two scalars have the same square class.  This gives
\eqref{eq:pointed-return-bits}.  The
unibranch $A_6$ normalization and its order-$23$ target intersection make
this the unique pointed $(11,15)$ node; changing the formal coordinate on
either branch does not change the normalizer-coset class.
\end{proof}

It remains to place the generic intersection and the local intersection at
the node in one geometric object.  We use a Ferrand pushout
\cite{Ferrand}.  After passing to the labeling torsor, let $\mathcal W$ be
the set of all involutions in $S_{23}$ of cycle type
$1^7 2^8$.  For $w\in\mathcal W$, let $B_w$ be its set of sheet orbits, and
set
\[
 \mathcal I=\{(w,b'):w\in\mathcal W,\ b'\in B_w\},
 \qquad E_x=B_x\times\mathcal I.
\]
The endpoint incidence $\mathcal L_D$ is a subcorrespondence of $E_x$.
On the conductor subscheme, the gluing morphism is induced by the canonical
surjection
\begin{equation}
 p:E_x\longrightarrow Q,\qquad (b,w,b')\longmapsto b'.
\label{eq:pinch-map}
\end{equation}
where $Q$ is the set of one- and two-element sheet subsets.  Thus the
normalization retains $(b,w,b')$, whereas its image in the conductor retains
only $b'$.

The local algebra makes this statement precise.  For a finite surjection
$p:E\to Q$, a DVR $R$ with residue field $k$, and fraction field $K$, form
the Ferrand pushout
\[
 \mathcal P_p=
...
\label{eq:pinch-algebra}
\end{equation}
The $R$-algebra $A_p$ is finite free of rank $|E|$; the normalization of
$\mathcal P_p$ is $\operatorname{Spec}(R^E)$, the conductor subscheme in the
normalization is $\operatorname{Spec}(k^E)$, and its image in
$\mathcal P_p$ is $\operatorname{Spec}(k^Q)$.  The generic fibre has
coordinate algebra $K^E$, and the reduced closed fibre has coordinate
algebra $k^Q$.  At $q\in Q$, the nearby stalk with $\Lambda=\FF_2$ is
$\Lambda[p^{-1}(q)]$, with no higher nearby cohomology.  The specialization
unit and its dual counit are
\begin{equation}
 \Lambda\longrightarrow\Lambda[p^{-1}(q)]
       \longrightarrow\Lambda,\qquad
 1\longmapsto\sum_{e\mapsto q}[e],\quad
 \sum a_e[e]\longmapsto\sum a_e.
\label{eq:pinch-unit-counit}
\end{equation}

More invariantly, let
$\nu:\operatorname{Spec}(R^E)\to\mathcal P_p$ be the normalization and let
$i:\operatorname{Spec}(k^Q)\hookrightarrow\mathcal P_p$ be the conductor
subscheme.  With $p:\operatorname{Spec}(k^E)\to\operatorname{Spec}(k^Q)$,
the normalization--conductor (Mayer--Vietoris) triangle is
\[
 \Lambda_{\mathcal P_p}
 \longrightarrow R\nu_*\Lambda_{\operatorname{Spec}(R^E)}
       \oplus i_*\Lambda_{\operatorname{Spec}(k^Q)}
 \longrightarrow i_*p_*\Lambda_{\operatorname{Spec}(k^E)}
 \xrightarrow{+1}.
\]
Consequently, after pushforward along $p$, endpoint vectors $\alpha,\beta$
pair at $q$ as
\begin{equation}
 \left(\sum_{e\mapsto q}\alpha_e\right)
 \left(\sum_{e\mapsto q}\beta_e\right).
\label{eq:bucket-pairing}
\end{equation}
whereas the normalization carries the diagonal coordinate pairing
$\sum_e\alpha_e\beta_e$.

\begin{lemma}[Trace across the normalization--conductor square]
\label{lem:pinched-tree-trace}
On a split pointed stable model, let $\Lambda[E]$ be the permutation local
system obtained by transporting the label set $E$ along the dual graph.
Apply the two endpoint correspondences $\mathcal L_{D_y}$ and
$\mathcal L_{D_z}$, and identify the labels on the conductor by $p:E\to Q$.
After applying nearby cycles, their Lefschetz--Verdier pairing is the
diagonal coordinate pairing on the normalization and the fibre-sum pairing
\eqref{eq:bucket-pairing} on the conductor.  Since the dual graph of the
semistable target is a tree, the total trace is the sum of the normalization
term at the wild end and the conductor term at the finite end.
\end{lemma}

\begin{proof}
At $q\in Q$ the strict local generic fibre is the discrete set
$p^{-1}(q)$.  Thus its degree-zero nearby stalk is
$\Lambda[p^{-1}(q)]$ and its higher cohomology vanishes.  The two arrows in
\eqref{eq:pinch-unit-counit} give the pairing
\eqref{eq:bucket-pairing} on the conductor; on the normalization one has the
diagonal coordinate pairing on the same stalk.  These are the two terms in
the normalization--conductor distinguished triangle for $\mathcal P_p$.
The triangle is functorial for the two cohomological correspondences, and
additivity of categorical traces identifies the total pairing with the sum
of its normalization and conductor terms.

The stalk complex on the dual graph is
\[
 C^0(\Gamma,\Lambda[E])\longrightarrow
 C^1(\Gamma,\Lambda[E]),
\]
with branch-difference differential.  Trivialize it along the unique paths
in the tree.  Each internal vertex value then occurs in two adjacent
branch differences and cancels in characteristic $2$.  Subdividing an edge
inserts the same value twice, so the conclusion is independent of the
chosen semistable refinement; only the two stated endpoint terms remain.
\end{proof}

Thus \eqref{eq:bucket-pairing} is the conductor term in the
normalization--conductor triangle after applying nearby cycles; it is only
one term in the specialized pairing.

For $\mathcal L_D$, the fibre of \eqref{eq:pinch-map} has size $77$ over a singleton and
$8$ over a two-element orbit.  Indeed, double counting the seven fixed
sheets over the $253$ involutions gives $253\cdot7/23=77$.  On ordered
distinct pairs the two $D$-orbits are exchanged by reversing a pair, so
double counting the sixteen moved entries of each involution gives $8$.
Thus the two endpoint graph sums have coefficients
\begin{equation}
 77^2=5929\quad\text{and}\quad8^2=64.
\label{eq:branch-coefficients}
\end{equation}
on the diagonal and transposition branches of the completed tame
self-fibre product.  Since that product is
\[
 R[[u,v]]/((u-v)(u+v)),
\]
the local intersection multiplicity changes by
\begin{equation}
 5929-64=5865\equiv1\pmod2.
\label{eq:node-unit}
\end{equation}
The eight moved blocks give the affine eight-point cycle and the ordinary
diagonal gives the fixed Fano heptad.  The degree-three Fano incidence
correspondence cancels the heptad against the seven nonorigin affine
directions over $\FF_2$, leaving one intrinsic finite-node term.  Explicitly,
after the pointed tail selects an origin $o\in A_x$, write
$U=O_2(\Cx)/\langle x\rangle$.  Then
$H_x=\PP(U^*)$ and $A_x-\{o\}$ is indexed by $\PP(U)$.  The incidence
$\ell(u)=0$ has degree three over either seven-point set, hence degree one
modulo $2$; its pushforward identifies the heptad with
$A_x-\{o\}$.  Adding the full affine cycle leaves $[o]$.  The conclusion is
intrinsic to the pointed tail, which fixes $o$; no point--line bijection is
chosen.

At the wild end of the dual graph, the normalization retains the labels.
The two fixed-point sets are $x^D$ and $(x^D)^n$: they agree for
$\chi_{\mathrm{gl}}(n)=0$ and lie in the disjoint $2A$ classes of the two
adjacent natural Mathieu copies for $\chi_{\mathrm{gl}}(n)=1$.  By
\cref{lem:fano-affine-odd}, their contribution on the normalization is
\begin{equation}
 1+\chi_{\mathrm{gl}}(n).
\label{eq:returned-normalized}
\end{equation}

By \cref{lem:pinched-tree-trace}, the closed-fibre trace is the sum of
\eqref{eq:node-unit} and \eqref{eq:returned-normalized}.  There is no
additional local term: the different exponent is even at every tamely
ramified attachment of odd order, and at a wild $23{:}11$ point of the
natural degree-$23$ quotient it is $22+2h$, which is also even.  Hence the
closed-fibre trace is
\begin{equation}
 (1+\chi_{\mathrm{gl}}(n))+1=\chi_{\mathrm{gl}}(n).
\label{eq:connector-special}
\end{equation}

\begin{proof}[Proof of \cref{thm:connector}]
For each closed point $P$ above $23$, let $R$ be the completed local ring of
the normalized integral Hurwitz scheme at $P$ and put $S=\operatorname{Spec}R$.
After the finite extension used to obtain the stable marked model, the
auxiliary Ferrand pushout is separated of finite type over the resulting
Henselian trait and finite over the proper pointed stable curve.  In
particular, the support of the fixed-point correspondence is proper over
$S$.  Take $\Lambda=\FF_2$.  The finite permutation complexes used above
belong to $D_{c\mathrm{ft}}$, and $2$ is invertible on $S$.

Corollary~3.10 of Lu--Zheng specializes the Lefschetz--Verdier pairing of
the two cohomological correspondences, while their Proposition~3.12 makes
this specialization compatible with proper pushforward of the graph
correspondences \cite[Corollary~3.10 and Proposition~3.12]{LuZheng}.  The
generic trace is
\eqref{eq:connector-generic}, while the closed-fibre trace is
\eqref{eq:connector-special}.  Therefore
\[
 \epsilon(\Theta)=\chi_{\mathrm{gl}}(n).
\]
By \cref{comp:pointed-stable-returns}, $\chi_{\mathrm{gl}}(n)$ and
$e_{\mathrm{sing}}$ agree on the three factors of the reduced closed fibre.
The construction is invariant under the patching-data torsor and under
simultaneous relabeling, so the equality descends from the splitting
extension.  Idempotents lift uniquely through the complete local algebra,
which proves $\epsilon(\Theta)=e_{\mathrm{sing}}$.
\end{proof}

\smallskip
The fixed-point argument is now complete.  The next two sections record
supporting arithmetic on the distinguished cover: first its explicit minimal
projection, then the Fano and affine arithmetic of its branch fibre.  Neither
section is needed to identify the $1+6$ Hurwitz algebra, and their unpointed
data alone do not prove \cref{thm:connector}.
...
what is already supplied by the branch-fibre structures from the additional
pointed data used in the comparison of \S\ref{sec:connector}.
...
comparison of \S\ref{sec:connector} may be formulated entirely in the
minimal-degree model.
The branch fibre by itself does not distinguish the degree-one Hurwitz
component.  The additional input is a common pointing of the finite and wild
ends of the characteristic-$23$ stable map; \S\ref{sec:connector} supplies
that input and constructs the resulting comparison.
...
problem.  This is precisely why the comparison in
\cref{sec:connector} retains a common pointing through stable reduction.
...
\section{A remaining open question}
\label{sec:open}

The pointed comparison identifies the idempotent defined by the
singular-point coordinate with the relative-transporter augmentation.
One arithmetic classification problem remains: classify the
...
formal annulus at the ramified $A_6$ node used to compute the gluing
character.  The main targets are:
...
Gluing characters and specialization of the nearby-cycle pairing &
...
\path{notes/certify_pinched_tag_nearby_cycle.py} & GAP, Python\\
\bottomrule
\end{longtable}
}
...
\item We determine the local decomposition and pointed reductions of that
  ...
  relative-transporter invariants, and compare their component idempotents
  by pointed stable reduction and specialization of a Lefschetz--Verdier
...
proves its agreement with the relative-transporter augmentation from pointed
stable $M_{23}$-maps and the normalization--conductor trace.  It uses the
explicit characteristic-$23$ local equations and the general stable-reduction
formalism, but not the identification of those local points with the seven
certified complex Nielsen classes.
