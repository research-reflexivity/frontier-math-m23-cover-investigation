...
intersection number is $\epsilon(\Theta)$ and analyze a proposed
characteristic-$23$ specialization.  A direct nearby-cycle calculation shows
that the simplest pinching construction retains the sheet labels and does
not produce the required quotient pairing.  We isolate the resulting
Bockstein comparison as a precise open problem rather than use it in the
...
motivate the candidate geometric comparison, but the branch fibre alone does
not distinguish the fixed Hurwitz point.
...
identity supplies the finite permutation sets in a candidate cohomological
explanation of \cref{thm:connector}.  We make the generic correspondence
precise and compute its proposed local specialization.  The nearby-cycle
stalk keeps the individual labels, however, so the proposed specialization
does not by itself prove the theorem.  This distinction is recorded
explicitly in \cref{sec:connector}.
...
distinguished cover.  The second set contributes geometry to a possible
conceptual explanation, but it is not itself the moduli scheme and does not
by itself distinguish the fixed point.
...
construct the generic side of a possible cohomological explanation and show
why a natural pinching shortcut does not supply its special side.  Supporting
...
The missing step is local.  A tempting Ferrand pushout identifies several
...
The exact local numbers do point to a more refined possibility.  Before
reduction modulo $2$, the difference of the two branch coefficients is
divisible by $2$ but not by $4$.  A relative two-term complex and its
Bockstein boundary could therefore retain the missing parity.  Constructing
that complex and proving that its generic and special trace classes agree is
the precise geometric problem left open here.  The paper records the valid
generic correspondence, the local calculation, and the obstruction, without
using the proposed Bockstein connector in any theorem.
...
\subsection{A candidate cohomological explanation}
\label{sec:connector}

We now separate a valid generic construction from the unresolved
specialization step.  This subsection is not used in the proof above.
...
\subsubsection{The remaining Bockstein problem}
...
\]
This is a genuine Bockstein in the normalization sequence.  It is not the
ordinary nearby-cycle pairing of \eqref{eq:connector-generic}.

\begin{conjecture}[Relative Bockstein connector]
\label{conj:bockstein-connector}
There is a self-dual two-level relative complex, schematically of the form
\[
 K=[A_{\mathrm{graph}}\longrightarrow B_{\mathrm{quotient}}],
\]
with cohomological packet maps whose generic trace is
$1+\epsilon(\Theta)$ and whose specialized Bockstein trace is the finite
node class plus the returned wild class.  Under the pointed
characteristic-$23$ identification, the latter trace is
$1+e_{\mathrm{sing}}$.
\end{conjecture}

To prove the conjecture one must construct the two packet maps on $K$, make
the duality pairing explicit, and verify that nearby cycles carry that same
pairing to the Bockstein class just computed.  The finite group and local
algebra certificates verify the displayed ingredients, but they cannot
supply these missing sheaf-theoretic maps.  In particular, no result of
Lu--Zheng is invoked here to identify the tagged and untagged pairings.

The component comparison is therefore complete by exact arithmetic, while
its branch-cycle-free cohomological explanation remains open.  The next two
sections record supporting arithmetic on the distinguished cover: first its
explicit minimal projection, then the Fano and affine arithmetic of its
branch fibre.
...
data that a future geometric comparison would require.
...
candidate construction of \S\ref{sec:connector} may be formulated entirely
in the minimal-degree model.
The branch fibre by itself does not distinguish the degree-one Hurwitz
ends of the characteristic-$23$ stable map together with a relative complex
that transports the Bockstein pairing.  The latter construction remains
open.
...
\cref{sec:connector} is stated as a conjectural Bockstein connector rather
than as a consequence of the unpointed data.
...
The most immediate geometric problem is \cref{conj:bockstein-connector}:
construct the relative self-dual two-level complex and prove that its generic
packet pairing specializes to the computed mod-$4$ node class and the
pointed wild return.  This would replace the exact branch-cycle crosswalk in
the proof of \cref{thm:connector} by an intrinsic specialization argument.
...
formal annulus at the ramified $A_6$ node used in the candidate connector.
...
Finite and local ingredients for the candidate Bockstein connector &
...
\path{notes/certify_pinched_tag_finite_identities.py} & GAP, Python\\
\bottomrule
\end{longtable}
}
...
\item We determine the local decomposition and pointed reductions of that
  ...
  the exact reconstructed algebra.  We construct the generic side of a
  possible cohomological explanation and isolate its unresolved relative
  Bockstein specialization.
...
generic incidence correspondence and the characteristic-$23$ local equations
suggest a branch-cycle-free explanation, but the nearby-cycle stalk retains
the sheet labels.  The requisite Bockstein comparison is therefore stated as
\cref{conj:bockstein-connector}, not claimed as a theorem.
