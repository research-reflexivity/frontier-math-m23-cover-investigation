...
\hypersetup{
  ...
  pdfsubject={Exact reconstruction, component idempotents, and reduction at 23 of a finite M23 Hurwitz scheme}
}
...
\begin{abstract}
Huang, Jackson, Lee, Poonen, Pries, and Zhang constructed a degree-$23$
cover with regular Galois closure of group $M_{23}$ and identified a
Galois-fixed class in its seven-element inner Nielsen class.  We reconstruct
the associated finite \'etale Hurwitz scheme exactly.  Over
$K_0=\QQ(\sqrt{-23})$, its coordinate algebra is $K_0\times L$, where
$[L:K_0]=6$, and the relative Galois closure of $L/K_0$ has group $S_6$.
Exact canonical models, degree-$23$ maps, and certified branch cycles
realize all seven classes.  A finite-group invariant and an idempotent
obtained from pointed reduction at $23$ both select the degree-six
component; their equality follows from the certified branch-cycle
matching.  We give a coordinate-independent local interpretation of this
idempotent: it is the complement of the Frobenius-fixed locus on the
reduced closed fibre, and it lifts to the splitting locus of an
unramified quadratic cover of the normalized integral model.
An independent comparison by relative cohomological correspondences
remains open.  For the distinguished cover, we construct a minimal
degree-$4$ projection and a primitive bidegree-$(23,4)$ equation.  The
Fano plane and affine $\FF_2^3$-torsor in its branch fibre give a
$2^4{:}\PSL_3(2)$ field tower.  Finally, every integer specialization
$t\equiv2\pmod{319}$ has Galois group $M_{23}$; controlled ramification
gives an infinite family for which every finite subfamily is linearly
disjoint.  Exact data and verification scripts accompany the results.
\end{abstract}
...
Seven covers with the same branch-cycle classes need not have the same
arithmetic.  In the construction of Huang et al.\ \cite{HJLPPZ}, the inner
Nielsen class for $(2A,23A,23B)$ in $M_{23}$ has seven elements.  One class
is fixed by the arithmetic Galois action considered there and gives their
cover over $\QQ$.  Its Galois closure over $\QQ(T)$ is regular: $\QQ$ is
algebraically closed in that function field.  The existence and descent of
this cover are therefore our starting point, not conclusions of the
present paper.

We study the arithmetic of the entire seven-point Hurwitz scheme and its
reduction at $23$.  The exact reconstruction separates a degree-one
component from a degree-six component.  A finite-group calculation and a
pointed reduction calculation then define the same component idempotent.
We prove this agreement by certified branch-cycle matching and give the
local idempotent an interpretation in terms of Frobenius and an
unramified quadratic extension.

Put $K_0=\QQ(\sqrt{-23})$, over which the ordered classes $23A,23B$ are
defined.  The fixed class of \cite{HJLPPZ} becomes the unique
$K_0$-rational point of the inner Hurwitz scheme below.  Its further
descent to a cover over $\QQ$ remains the result of \cite{HJLPPZ}.
...
then two separately defined ways to detect its complement.  The first
uses the augmentation $\epsilon(\Theta)$ of a finite group-algebra element.
The second evaluates a normalization parameter at a distinguished
singular point of each pointed characteristic-$23$ reduction and gives an
idempotent $e_{\mathrm{sing}}$.  Their comparison takes place in the
...
Reduction at $23$ gives the second idempotent a simple local
interpretation.  The normalized integral Hurwitz model has three closed
points, with residue degrees $1,2,2$ and ramification indices $1,1,2$.
The $23$-power Frobenius fixes precisely the residue-degree-one component.
Its complement is also the locus on which the unramified quadratic
extension splits.  These descriptions are independent of the chosen
normalization coordinate.  \Cref{thm:frobenius-criterion} gives the general
local statement, and \cref{cor:frobenius-hurwitz} applies it to this model.

\smallskip
The finite-group invariant has a complementary interpretation inside the
distinguished cover.  Let
...
identity realizes the finite-group invariant as a finite incidence
intersection.  Extending this interpretation to a relative
specialization argument remains open; \cref{sec:connector} explains
why the elementary nearby-cycle calculation is insufficient.

The Fano plane contributes through this incidence calculation: it lies in
the branch fibre of one cover, whereas the Hurwitz scheme parametrizes
all seven covers.
...
and explains the scope of their comparison.  The exact computations are
followed by the local Frobenius criterion and the remaining obstacle to
an independent geometric comparison.  The minimal
...
interpret the local idempotent by Frobenius and unramified quadratic
splitting, using standard facts about henselian rings and finite fields.
This interpretation does not remove the branch-cycle identification from
the comparison with $\Theta$.  Supporting
...
\]
The component comparison concerns two separately defined functions on the
...
\cref{thm:connector}.  The proof combines exact algebra with computational
certificates.  Its dependence on the branch-cycle matching is precise:
that matching places the finite-group values on the two factors of
$K_0\times L$.

The local idempotent has a coordinate-independent description.
Let $R$ be the completion of the ring of integers of $K_0$ at its prime
above $23$,
and normalize $R$ in $A_{\mathrm H}\otimes_{K_0}\operatorname{Frac}R$.
Its reduced closed fibre has coordinate algebra
\[
 B\simeq\FF_{23}\times\FF_{23^2}\times\FF_{23^2}.
\]
The scheme-theoretic fixed locus of $23$-power Frobenius is just the
first factor.  Its complementary idempotent lifts uniquely to $e_6$.
Equivalently, the unramified quadratic extension of $R$ splits on
precisely the other two components.  We prove these assertions, including
their general local form, in \cref{sec:frobenius-criterion}.

Thus the singular-position calculation detects residue-field arithmetic,
not merely the geometric type of the source singularity.  The two
$E_8$ reductions have different residue degrees and are distinguished by
Frobenius.  This is a local interpretation of the idempotent already
computed, not a new proof of its agreement with the finite-group
invariant.

For that stronger comparison, the Fano--affine odd fixed-point identity
provides a finite incidence formula for $\epsilon(\Theta)$.
However, in a finite pinching model the nearby-cycle stalk retains one
basis vector per generic sheet, and its pairing is
$\sum_e\alpha_e\beta_e$, not
$(\sum_e\alpha_e)(\sum_e\beta_e)$.  The integral quadratic identities
recorded in \cref{sec:connector} do not construct a relative
correspondence or a determinant line carrying the required extra data.
A specialization proof identifying the two invariants without the
certified branch-cycle matching remains an open question.
...
We retain the following finite identities because they identify the input
that a proof independent of branch-cycle matching would have to transport.
They do not construct relative correspondences on a semistable
$M_{23}$-cover.
...
The principal open question is to prove
$\epsilon(\Theta)=e_{\mathrm{sing}}$ by specialization, independently of
the certified branch-cycle matching.  This requires a relative geometric
realization of the two finite incidence correspondences, retaining the
separate integral multiplicities $\mathcal C$ and $T_D$, and a proof that
the resulting special invariant is the Frobenius or quadratic-splitting
idempotent of \cref{cor:frobenius-hurwitz}.  A functorial quadratic or
hermitian construction is a possible approach, not an object constructed
in this paper.  Ordinary nearby-cycle duality and the finite identities in
\cref{sec:connector} do not establish that step.
Approaches via bad reduction of three-point covers \cite{Wewers} or
logarithmic nearby cycles \cite{Nakayama} would still have to specify
the relative objects and prove the compatibility of their multiplicities.

The local component criterion applies to any finite normal algebra over
a local field's ring of integers.  A separate question is when a
group-theoretic invariant of other Nielsen classes can be proved to
detect its residue degrees without first matching every exact cover to
a branch-cycle triple.
...
the constituent fields and discriminants of \cref{thm:ramification}; in
...
\item We determine the local decomposition and pointed reductions of that
  ...
  the exact reconstructed algebra.  We interpret the local idempotent as
  the complement of a Frobenius-fixed locus and as an unramified
  quadratic-splitting locus.  We prove the local component criterion
  underlying this interpretation and specify why the finite incidence
  identities do not yet give a comparison by specialization.
...
coordinate gives a characteristic-$23$ criterion on the specified pointed
models separating the degree-one and
...
Frobenius interpretation in \cref{cor:frobenius-hurwitz} is independent of
the singular-position coordinate, but it does not remove the matching from
the comparison with $\Theta$.  No relative quadratic determinant line or
second specialization proof is asserted.  In particular, the elementary
nearby-cycle stalk retains the individual sheets, and the unrestricted
coefficient Bockstein calculation does not recover the integral quadratic
invariant.
