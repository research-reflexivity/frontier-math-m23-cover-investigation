...
\title[The Galois-fixed point on an $M_{23}$ Hurwitz scheme]
  {The Galois-fixed point on an $M_{23}$ Hurwitz scheme:\\
   exact reconstruction and characteristic-$23$ geometry}
...
\hypersetup{
  pdftitle={The Galois-fixed point on an M23 Hurwitz scheme: exact reconstruction and characteristic-23 geometry},
  pdfauthor={François Le Lay},
  pdfsubject={Exact arithmetic reconstruction and characteristic-23 geometry of the Galois-fixed point on an M23 Hurwitz scheme}
}
...
point, characteristic-$23$ reduction, Fano plane, inverse Galois
problem}

\begin{document}

\begin{abstract}
Huang, Jackson, Lee, Poonen, Pries, and Zhang construct a regular degree-$23$
$M_{23}$-cover from a seven-element inner Nielsen class and identify one
class fixed by the relevant arithmetic Galois action, which yields their
cover over $\QQ$.  We determine exactly the corresponding finite \'etale
Hurwitz scheme of degree $7$ and use its characteristic-$23$ geometry to
distinguish that class from the other six.

\smallskip
Over $K_0=\QQ(\sqrt{-23})$, the coordinate algebra of the finite inner
Hurwitz scheme is
\[
 A_{\mathrm H}=K_0\times L,\qquad [L:K_0]=6.
\]
The relative Galois closure of $L/K_0$ has group $S_6$.  Exact canonical
models and degree-$23$ maps over $A_{\mathrm H}$, together with certified
branch cycles, recover every Nielsen class and identify the degree-one
factor with the distinguished cover.

\smallskip
At the prime above $23$, the normalization of the integral Hurwitz scheme
has three closed points.  On each pointed closed fibre, the normalization
parameter of a distinguished singular point has minimal polynomial
\[
 (u-16)(u^2+1)(u^2+u+1)=0
\]
and constructs an idempotent $e_{\mathrm{sing}}$ separating the degree-one
and degree-six components.  Independently, a finite-group construction with
transporter elements in $M_{23}$ produces an augmentation
$\epsilon(\Theta)$ with the same two values on the seven exact covers.
The certified identification of those covers with the seven Nielsen classes
therefore proves
\[
 \epsilon(\Theta)=e_{\mathrm{sing}}.
\]
We also construct a cohomological correspondence on the generic fibre whose
mod-$2$ intersection number is $\epsilon(\Theta)$.  A direct calculation of
nearby cycles shows that specialization preserves the basis indexed by the
individual sheets.  A separate mod-$4$ calculation shows that the ordinary
coefficient Bockstein vanishes after the chosen integral representatives are
forgotten.  We therefore equip the normalization complex with two integral
multiplicity vectors on its graph components and form its determinant line,
together with the induced symmetric form and $\ZZ/2$-grading.  Its generic
degree is $1+\epsilon(\Theta)$; on the special fibre the degree is the sum of
a contribution $1$ from the node over the finite branch point and the
contribution $e_{\mathrm{sing}}$ from the transition between the two marked
wild branches.  This gives a second proof of the displayed equality,
independent of the certified identification of the seven characteristic-zero
branch cycles.  In particular, it does not replace the basiswise nearby-cycle
pairing by its image under augmentation.

\smallskip
Two further calculations describe the distinguished cover itself.  We
explicitly realize the degree-$4$ projection whose existence and minimality
were proved by Huang et al., obtaining a primitive bidegree-$(23,4)$
equation.  In its branch fibre above $T=0$, the seven unramified points form
a Fano plane, the eight colliding pairs form an affine $\FF_2^3$-torsor,
and their arithmetic gives a $2^4{:}\PSL_3(2)$ field tower.  These structures
enter the relative determinant calculation, but the branch fibre alone
does not distinguish the fixed Hurwitz point.

\smallskip
As a separate application, every integer specialization
$t\equiv2\pmod{319}$ of the minimal equation has Galois group $M_{23}$, and
controlled ramification selects an infinite mutually independent subfamily.
...
The construction of Huang et al.\ begins with an inner Nielsen class of
seven elements in $M_{23}$, with branch-cycle classes $(2A,23A,23B)$.  One
class is fixed by the arithmetic Galois action considered there and produces
their regular cover over $\QQ$; here regular means that $\QQ$ is algebraically
closed in the Galois closure over $\QQ(T)$.  The existence and rationality of
this class are known.  The question addressed here is why the corresponding point is
also distinguished by the intrinsic characteristic-$23$ geometry of the
moduli problem.

Our first main result determines the finite \'etale scheme and its coordinate
algebra exactly.  Put $K_0=\QQ(\sqrt{-23})$.

To fix the field of definition in the terminology, ``Galois-fixed'' in the
title refers to the class fixed by the arithmetic action in \cite{HJLPPZ}.
After the ordered classes $23A,23B$ are defined over $K_0$, the theorem below
identifies the corresponding moduli point as the unique $K_0$-rational point
of the inner Hurwitz scheme.  Its descent to a cover over $\QQ$ is the result
of \cite{HJLPPZ} recalled above.

\begin{theorem}[Exact arithmetic of the finite inner Hurwitz scheme]
\label{thm:hurwitz-summary}
Let $\mathcal H_{\mathrm{in}}$ be the reduced inner Hurwitz scheme of
normalized three-point covers with branch-cycle classes $(2A,23A,23B)$.
...
Thus the distinguished geometric point is the unique $K_0$-rational point
of $\mathcal H_{\mathrm{in}}$; equivalently, it is the degree-one
open-and-closed subscheme in the canonical $1+6$ decomposition.  There are
then two independent ways to detect its complement.  The finite-group side
gives the augmentation $\epsilon(\Theta)$ of an explicitly defined
group-algebra element.
The geometric side evaluates a normalization parameter at a distinguished
singular point of each pointed characteristic-$23$ reduction and gives an
idempotent $e_{\mathrm{sing}}$.  The central result identifies them in the
exactly reconstructed coordinate algebra.

\begin{theorem}[Agreement of the two component idempotents]
\label{thm:connector}
Let $e_{\mathrm{sing}}$ be the idempotent defined by the singular-point
coordinate in \cref{prop:singular-position-idempotent}, and let
$\epsilon(\Theta)$ be the
augmentation of the group-algebra element $\Theta$ defined in
\cref{comp:relative-transporter-descent}.  Then
\[
 \epsilon(\Theta)=e_{\mathrm{sing}}
\]
in the coordinate algebra of $\mathcal H_{\mathrm{in}}$.  Under the
isomorphism $A_{\mathrm H}=K_0\times L$, both sides equal the primitive
idempotent $e_6=(0,1)$.
\end{theorem}

The characteristic-$23$ geometry behind the second idempotent uses a feature
visible inside the distinguished cover.  Let
$X\to\PP^1_T$ be the degree-$23$ cover constructed in \cite{HJLPPZ}.  Its
splitting field over $\QQ(T)$ is regular with Galois group $M_{23}$, its
source curve has genus $4$, and its branch points are
\[
 0,\qquad \sqrt{-23},\qquad -\sqrt{-23}.
\]
The inertia involution at $T=0$ has cycle shape $1^7 2^8$: seven sheets
remain fixed and eight pairs collide.  These two sets support respectively
a Fano plane and an affine $\FF_2^3$-torsor.  Their common odd fixed-point
identity is the finite group-theoretic input to a second proof of
\cref{thm:connector}.  The ordinary nearby-cycle stalk keeps the basis
indexed by the individual sheets and its coefficient Bockstein vanishes.  In
\cref{sec:connector} we instead retain integral multiplicities on the
relevant graph components, construct a determinant line with its induced
symmetric form and $\ZZ/2$-grading from the normalization--conductor complex
of the semistable model, and compare its two fibres
without using the exact identification of the seven branch-cycle triples.

Two distinct seven-element Galois sets must therefore be kept separate.  The
seven geometric points of $\mathcal H_{\mathrm{in}}$ parametrize seven
covers.  The seven unramified points above $T=0$ lie inside only the
distinguished cover.  The second set contributes to the relative determinant
calculation, but it is not itself the moduli scheme
and does not by itself distinguish the fixed point.

To make the arithmetic within the distinguished cover exact, we also carry
out the minimal-coordinate construction anticipated by Huang et al.  Write
$F(T,V)\in\ZZ[T,V]$ for their integral source equation.
...
The coordinate change is regular across the structured branch fibre and
preserves both of its residue fields.
...
point} means the fibre of $X\to\PP^1_T$ above $0\in\PP^1(\QQ)$; it does not
assert that its points are $\QQ$-rational.  This is a characteristic-zero
branch fibre.  We reserve \emph{closed fibre} for a model over a local base
and use \emph{reduction} for passage modulo a prime.

The main argument is organized as follows:
\[
 \text{degree-$7$ Hurwitz algebra}
 \longrightarrow
 \text{seven exact maps and certified branch cycles}
 \longrightarrow
 \epsilon(\Theta)=e_{\mathrm{sing}}.
\]
The pointed reduction at $23$ independently constructs
$e_{\mathrm{sing}}$.  \Cref{sec:mechanism} first compares the two idempotents
and then states the relative determinant construction used for a second
proof, before turning to the computational details.  The minimal
projection and branch-fibre arithmetic are supporting results on the
distinguished cover, and the specialization theorem is an independent
application.
...
Relative to this prior work, the central contributions are the exact
$K_0\times L$ Hurwitz algebra, its $S_6$ Galois closure and seven exact maps,
the characteristic-$23$ idempotent defined by the distinguished singularity,
and its exact
identification with the augmentation of $\Theta$.  We additionally
construct a relative determinant line with a symmetric form and grading,
giving a comparison
independent of the branch-cycle identification, after showing why ordinary
nearby-cycle duality and the Ferrand pushout alone cannot supply it.  Supporting
results give the explicit minimal-degree equation and ruling field, the
arithmetic and obstruction theory of the distinguished branch fibre, and the
congruence class modulo $319$ in which every specialization has group
$M_{23}$, together with controlled ramification.  The
detailed attribution, limitations, and data provenance are recorded in
\cref{sec:scope}.

\section{The two component idempotents}
\label{sec:mechanism}

There are two primitive idempotents in
\[
 A_{\mathrm H}=K_0\times L:
 \qquad e_1=(1,0),\qquad e_6=(0,1).
\]
The theorem of the paper compares two independently defined functions on the
seven geometric points.  The first is the augmentation
$\epsilon(\Theta)$ of a group-algebra element in
$\FF_2[2^3{:}\PSL_3(2)]$.  Its certified values are $0$ on the distinguished
Nielsen class and $1$ on the other six.  The second comes from the
normalization of the integral Hurwitz scheme at the prime above $23$.
Evaluation at the distinguished singular point has polynomial
\[
 (u-16)(u^2+1)(u^2+u+1),
\]
and the function taking the values $(0,1,1)$ on these three closed points is
$e_{\mathrm{sing}}$.

The exact reconstruction identifies the seven maps with the seven Nielsen
classes.  It follows that both functions are $e_6$, which proves
\cref{thm:connector}.  This proof is arithmetic and fully certified.  It is
not intrinsic to the pointed characteristic-$23$ model: the certified
branch-cycle matching is what places the finite-group values on the two
factors of $K_0\times L$.

The same equality may also be obtained from the pointed
characteristic-$23$ model.  For an involution $x\in2A$, its seven fixed
sheets form a set $H_x$ endowed with the Fano-plane incidence, and its
eight two-sheet orbits form an affine $\FF_2^3$-torsor $A_x$.  Every element
$c\in C_{M_{23}}(x)$ satisfies
\[
 |\Fix_{H_x}(c)|+|\Fix_{A_x}(c)|\equiv1\pmod2.
\]
Using this identity, finite incidence correspondences attached to the
normalizers at the two marked wild branches realize $\epsilon(\Theta)$ as a generic
intersection number.  This construction is given in
\cref{sec:connector}.

The local issue is subtle.  A Ferrand pushout identifies several generic
sections at a closed point.  Its nearby stalk at a quotient point
$q$ is not the one-dimensional constant sheaf: it is the full permutation
module
\[
 \FF_2[p^{-1}(q)].
\]
Consequently the specialized pairing of the two lifted sections is still the
coordinate pairing $\sum_e\alpha_e\beta_e$.  The fibre-sum product
$(\sum_e\alpha_e)(\sum_e\beta_e)$ appears only after an additional
unit--counit operation and is a different cohomological correspondence.  The
two pairings already differ on a two-element fibre, so they cannot be
identified by formal trace compatibility.

The exact local numbers determine the additional integral structure used
here.  Before
reduction modulo $2$, the difference of the two branch coefficients is
divisible by $2$ but not by $4$.  Its half modulo $2$ is the local value of
the quadratic function $Q$ defined in \cref{sec:connector}, evaluated on two
chosen integral sums of graph components.  Such a sum means a sum with
nonnegative integral coefficients; it is multiplicity data on the
normalization and not a Chow cycle on the curve.
The ordinary coefficient Bockstein nevertheless vanishes,
both in the unrestricted mapping cone of the graph-to-sheet map and in the rank-one horizontal
logarithmic nearby-cycle stalk.  The construction in
\cref{thm:quadratic-connector} therefore retains these integral
multiplicities as a chosen Lagrangian in the normalization--conductor complex
of the semistable model and uses the determinant line with its induced symmetric form and
$\ZZ/2$-grading.  The resulting line carries the quadratic refinement
without changing the ordinary nearby-cycle pairing.

\section{Exact Hurwitz arithmetic and the component comparison}
\label{sec:hurwitz-scheme}

\subsection{Coordinate algebra and canonical models}

We now turn to the seven-cover moduli problem itself.  The section has three
tasks: reconstruct and identify the finite Hurwitz scheme, construct its
component idempotents independently from finite-group and characteristic-$23$
data, and compare them in the exact coordinate algebra.  The normalized canonical
...
this shows that $N$ and $D$ have no common zero and proves the divisor
identity.
\end{proof}
...
evaluate the normalization coordinate at the point above $\tau_P$.
...
singularity defines $e_{\mathrm{sing}}=e_6$.  By
\cref{prop:relative-transporter-descent}, the augmentation is
$\epsilon(\Theta)=e_6$.  Hence
$\epsilon(\Theta)=e_{\mathrm{sing}}$ in $A_{\mathrm H}$.
\end{proof}

\subsection{A relative determinant calculation}
\label{sec:connector}

We first explain why ordinary nearby-cycle duality does not give the desired
comparison.  We then construct a graded symmetric line from a normalization
complex with integral multiplicities on its graph components.  Comparing its
generic and special fibres gives a second proof of \cref{thm:connector},
independent of the certified identification of the seven reconstructed
branch-cycle triples used above.
...
\subsubsection{The integral quadratic refinement}

At a split tame node, the normalization quotient sends branch coefficients
$(a_+,a_-)$ to $a_+-a_-$.  When these coefficients agree modulo $2$, the
chosen integral representatives determine the divided branch difference
\[
 \kappa(a_+,a_-)=\frac{a_+-a_-}{2}\pmod2.
\]
Put \(\mathcal C=2A(M_{23})\), let \(T_D=x^D\) be a normalizer orbit, and
let \(P_D=\mathcal C\setminus T_D\).  We regard their characteristic
functions as integral sums of graph components.  For the image of
\(P_D\) at the node over the finite branch point, the two exact branch
coefficients are
\[
 a_+=1078,\qquad a_-=112,
 \qquad \kappa=\frac{1078-112}{2}=483\equiv1\pmod2.
\]
This divided branch difference depends on the chosen integral
representatives.  It is neither the nearby-cycle pairing in
\eqref{eq:connector-generic} nor an ordinary coefficient Bockstein after
those representatives have been forgotten.

For the natural sheet representation \(\rho\), direct counting gives
\[
...
and hence
\[
 \rho(P_D)=112J+966I\equiv2I\pmod4.
\]
The congruent integral representative \(P_D-2\mathcal C\), however, maps to
\(-128J-1104I\equiv0\pmod4\).  Thus the coefficient Bockstein of the mod-$2$
class represented by \(P_D\) in the unrestricted graph-to-sheet mapping
cone is zero.  The class \(\kappa\) is retained only when the integral sums
\(\mathcal C\) and \(T_D\) are kept separately.

Logarithmic nearby cycles do not remove this obstruction.  At a smooth point
of a horizontal logarithmic divisor, Nakayama's formula
\cite[Theorem~3.5]{Nakayama} gives one free coefficient copy in degrees zero
and one.  Reduction from \(\ZZ/4\) to \(\FF_2\) is surjective in both degrees,
so the coefficient connecting homomorphisms vanish.  Taking direct sums
indexed by the graph components does not change this conclusion.

The nonzero invariant has a quadratic description.  On the even-weight
subspace of a binary permutation module set
\[
...
\]
In the permutation module on a common finite set containing the two
\(2A\)-classes considered below, one has
\[
 Q(T_{D_y}+T_{D_z})=1+\epsilon(\Theta),\qquad
 Q(P_D)=1.
\]
On a completed trait of the pointed local Hurwitz model at $23$, transport
the chosen affine frame from one marked wild branch to the other, and let
\(n\in N_{S_{23}}(D)=23{:}22\) be the resulting comparison element.  Let
\(q(n)\in\FF_2\) denote its image in
\(N_{S_{23}}(D)/D\), where \(D=23{:}11\).  The two orbits \(T_D\) and
\(T_D^n\) coincide when \(q(n)=0\).  When \(q(n)=1\), they are disjoint
subsets of \(2A(M_{23})\) and \(2A(M_{23}^n)\), respectively.  Consequently
\[
 Q(T_D+T_D^n)=q(n),\qquad
 Q(\mathcal C+T_D^n)=1+q(n).
\]
The cross-term between \(P_D\) and \(T_D+T_D^n\) is zero in both cases, so
the last equality is exactly the polarization identity above with the
contribution \(Q(P_D)=\kappa=1\) from the node over the finite branch point.
The pointed reduction computations give \(q(n)=0,1,1\) on the three closed
characteristic-$23$ branches, which are the values of
\(e_{\mathrm{sing}}\).

We now express this refinement in the Picard groupoid of graded symmetric
lines.  Let \(R\) be a ring in which \(2\) is invertible and let \(E\) be a
finite free permutation module of even rank \(2m\), with its standard
orthonormal form.  We use the notation
\begin{equation}
 \operatorname{qdet}(E)
  =\bigl(\det E,(-1)^m\det\langle\ ,\ \rangle_E,
          m\bmod2\bigr).
\label{eq:hdet-definition}
\end{equation}
The first entry is the determinant line, the second is the square class of
its induced symmetric form, and the last is its $\ZZ/2$-degree.  The sign in
\eqref{eq:hdet-definition} is the usual Clifford-algebra sign:
if \(e_1,\ldots,e_{2m}\) is an orthonormal basis, then
\((e_1\cdots e_{2m})^2=(-1)^m\).  It is independent of the ordering and
extends by alternating tensor product to based virtual perfect complexes of
even Euler characteristic.

For two subsets \(A,B\) of a fixed finite indexing set, let \(e_A,e_B\) be
their commuting characteristic idempotents.  Then
\[
 s(A,B)=e_A+e_B-2e_Ae_B
\]
is the idempotent of their symmetric difference.  Put
\begin{equation}
 \mathfrak q(A,B)=\operatorname{qdet}(\operatorname{im}s(A,B)).
\label{eq:qor-definition}
\end{equation}
Its parity is \(Q(A+B)\).  If \(S,R\) are even subsets, the based
decomposition
\[
 \ZZ[S]\oplus\ZZ[R]
 \simeq \ZZ[S\mathbin\triangle R]
       \oplus\ZZ[S\cap R]^{\oplus2}
\]
gives the line-valued polarization law
\begin{align}
 \operatorname{qdet}(\ZZ[S])\otimes\operatorname{qdet}(\ZZ[R])
 &\simeq \operatorname{qdet}(\ZZ[S\mathbin\triangle R])
       \otimes\mathfrak p(S,R), \notag\\
 \mathfrak p(S,R)
 &=\operatorname{qdet}\bigl(\ZZ[S\cap R]^{\oplus2}\bigr),
 & \deg\mathfrak p(S,R)&=|S\cap R|\pmod2.
\label{eq:qor-polarization}
\end{align}
Thus \(\mathfrak q\) takes values in the Picard groupoid of
$\ZZ/2$-graded symmetric lines; it is not an ordinary square root of a
determinant line.

We shall use the following normalization statement.  It is formulated for
based perfect complexes together with integral multiplicity vectors on the
normalized graph components, rather than only their reductions modulo $2$.
The normalization sequence and determinant isomorphisms are classical.  The
additional assertion is that the divided branch differences determine, in a
functorial way, the node factors of the determinant line together with its
symmetric form and grading.

\begin{lemma}[Normalization and determinant gluing with integral multiplicities]
\label{lem:effective-normalization-gluing}
Let \(R\) be a ring in which \(2\) is invertible, and let \(X_0\) be a split
nodal curve whose dual graph \(\Gamma\) is a finite tree with a specified
set of marked vertices containing every vertex of valence different from
two.  Fix two marked vertices, denoted \(v_-\) and \(v_+\).  At every
unmarked vertex assume that the two restriction maps identify the same
basis and that the multiplicity vectors agree under this identification.  Let
\(\nu:\widetilde X_0\to X_0\) be the normalization, and let
\(\mathcal B_{\ZZ}\) be a finite free integral coefficient system with a
distinguished basis on
\(\widetilde X_0\), with based identifications of its two branch stalks at
every node; write \(\mathcal B=R\otimes_{\ZZ}\mathcal B_{\ZZ}\) for its
scalar extension.  For an integral multiplicity vector \(z\) on each
branch, put
\[
 \delta_e(z)=z_{e,+}-z_{e,-}
\]
after transporting both restrictions to the common basis at the node
\(e\).  For an integral multiplicity vector
\(m=\sum_wn_w[w]\), write
\[
 R[m]=\sum_wn_wR[w]
\]
in the Grothendieck group of finite based free \(R\)-modules; negative
multiplicities are represented by dual factors.  Thus \(z\) defines a
virtual based object before any reduction modulo \(2\).  Suppose that every
coefficient of every \(\delta_e(z)\) is even and that the virtual ranks of
the factors remaining after cancellation are even, so
\(\operatorname{qdet}\) is defined for them.
Then the following assertions hold.
\begin{enumerate}
\item The classes
\[
 \kappa_e(z)=\frac{\delta_e(z)}2\pmod2
 \quad\text{in }\FF_2[W_e]
\]
are the components of the branch-difference cochain divided by $2$.  Along the
unique path from the first marked vertex to the second, they satisfy the
vector identity
\begin{equation}
 \sum_e\kappa_e(z)=\frac{z_+-z_-}{2}\pmod2,
\label{eq:divided-normalization-telescope}
\end{equation}
after all bases have been transported to one endpoint.
Before reduction, the same identity gives, for the oriented path \(P\),
\begin{equation}
 \operatorname{qdet}\bigl(R[z_+]-R[z_-]\bigr)
 \simeq \bigotimes_{e\in P}\operatorname{qdet}
       \bigl(R[\delta_e(z)]\bigr),
\label{eq:normalization-line-transport}
\end{equation}
with the tensor factors ordered along \(P\).
\item The underlying normalization complex
\begin{equation}
 N_\Gamma^\bullet(\mathcal B)=
 \left[\bigoplus_v B_v\longrightarrow\bigoplus_eB_e\right],
 \qquad
 (db)_e=b_{e,+}-b_{e,-},
\label{eq:general-effective-normalization-complex}
\end{equation}
is a finite based perfect complex.  Apply the integral multiplicity vectors
to its normalization filtration in the Grothendieck category of based
perfect complexes.  The determinant isomorphism then factors the determinant
line, its symmetric form, and its grading into the marked-vertex factors
and the node factors \(\operatorname{qdet}(\delta_e(z))\).  Every unmarked internal based factor occurs
once covariantly and once contravariantly and has its canonical evaluation
trivialization.
\item This factorization is functorial under simultaneous based
relabelling, flat base change preserving the split normalization diagram,
branch exchange, gluing of marked trees, and semistable
subdivision.  Branch exchange dualizes the corresponding node factor;
subdivision inserts the based contractible complex
\([B\xrightarrow{1}B]\).
\end{enumerate}

For the relative form, let \(R\) be a strictly henselian discrete valuation
ring with \(2\in R^\times\), let \(A\) be the finite torsion-free
$R$-algebra obtained from the incidence construction above, with finite
normalization \(A^\nu\), and let
\(z_\eta\) be an idempotent on \(A^\nu\otimes_RK\).  Assume that
the conductor quotients are equipped with the based branch identifications
of the split system above, that the resulting differences are
coefficientwise divisible by \(2\), and that the virtual ranks remaining after cancellation are
even.  Then \(z_\eta\) extends uniquely to \(A^\nu\); its image is finite
projective over \(R\), and the conclusions above applied to the conductor
complex are the generic and special factorizations of one relative graded
symmetric line.
The statement applies componentwise to a finite ordered family of
idempotents; the line-valued polarization law then combines their
factorizations.
\end{lemma}

\begin{proof}
At one split node, the normalization sequence with coefficients in the
integral based module \(B_{\ZZ}\) is
\begin{equation}
 0\longrightarrow B_{\ZZ}\longrightarrow B_{\ZZ}\oplus B_{\ZZ}
 \xrightarrow{(b_+,b_-)\mapsto b_+-b_-}B_{\ZZ}\longrightarrow0.
\label{eq:split-node-coefficient-sequence}
\end{equation}
The kernel of \(\ZZ/4\to\FF_2\) is canonically \(2\FF_2\).  Hence, if the
branch difference of the distinguished integral coefficient is even,
its reduction modulo \(2\) lies in the kernel of the branch-difference map
and therefore descends across the node.  Write
\(z_{e,+}-z_{e,-}=2h_e\).  Then \(h_e\bmod2=\kappa_e(z)\) is precisely the
cochain measuring the failure of the two branch lifts to agree modulo
\(4\); equivalently, it is the first obstruction to descending the chosen
integral lift modulo \(4\).  Thus the local classes are the coefficients of
the branch-difference cochain divided by $2$.  On an oriented path
\(v_0,e_1,v_1,\ldots,e_r,v_r\), their integral numerators telescope:
\[
 (z_1-z_0)+(z_2-z_1)+\cdots+(z_r-z_{r-1})=z_r-z_0.
\]
Dividing by \(2\) and reducing modulo \(2\) proves
\eqref{eq:divided-normalization-telescope} as an equality of coefficient
vectors, not merely of their augmentations.  Before reduction, the same
telescoping equality is an equality of virtual based modules.  Applying
\(\operatorname{qdet}\) in path order proves
\eqref{eq:normalization-line-transport}.

The local sequences \eqref{eq:split-node-coefficient-sequence} assemble to
\eqref{eq:general-effective-normalization-complex}.  Cutting \(\Gamma\) at
the marked vertices decomposes it into chains whose internal vertices are
all unmarked.  Filter the complex on each chain by successively adjoining
its vertices and edges.  Each step is a short exact sequence of based
complexes.  Knudsen--Mumford determinant
additivity therefore identifies its determinant with the ordered tensor
product of the determinants of the graded pieces.  These filtrations are
based split: at every unmarked vertex the transition is a bijection of the
distinguished bases.  The induced symmetric form on the determinant line is
multiplicative on such orthogonal based sums, with the doubled-overlap
contribution recorded by
\(\mathfrak p\) in \eqref{eq:qor-polarization}.  At an unmarked internal vertex
the same based module occurs in two consecutive normalization triangles,
once in each dual cohomological position, so evaluation cancels the two
factors.  The remaining factors are the marked-vertex terms and the virtual
branch differences.  Applying the quadratic refinement to these same exact
sequences gives the asserted graded determinant factorization.

Gluing at marked vertices concatenates the two filtrations.  If the common
mark is forgotten after gluing and becomes an unmarked valence-two vertex,
its two factors evaluate against each other; if the mark is retained, so is
its factor.  Subdivision adds
\([B\xrightarrow{1}B]\), whose determinant has its canonical unit
trivialization.  Flat base change preserves
\eqref{eq:split-node-coefficient-sequence}; a based relabelling is an
isometry; and exchanging the branches negates the virtual difference and
therefore dualizes its line.  This proves the functorialities.

For the relative assertion, \(z_\eta\) satisfies the monic equation
\(X^2-X=0\), so it belongs to the integral closure \(A^\nu\); uniqueness
follows from the injection into the generic algebra.  Its image is a direct
summand of the finite torsion-free \(R\)-module \(A^\nu\), hence finite
projective.  Write \(\mathfrak c\) for the conductor.  The Ferrand square
\[
 A=A^\nu\times_{A^\nu/\mathfrak c}A/\mathfrak c
\]
and its module sequence \cite[Section~2.2]{Ferrand} supply the two branch
restriction maps to each common based conductor module.  Their difference
defines the relative complex
\eqref{eq:general-effective-normalization-complex}; when the difference
vanishes, its kernel is exactly the descended module, and in general its
cone records the descent defect.  The normalization terms are finite flat
over the trait, while the conductor terms are finite over its discrete
valuation ring and therefore have projective dimension at most one.
The resulting complex is therefore perfect.  Under a flat base change
preserving this conductor diagram, derived base change and determinant base
change identify its two fibre factorizations with those of one filtered
relative complex and one determinant line.
\end{proof}

\begin{theorem}[A relative determinant line with symmetric form and grading]
\label{thm:quadratic-connector}
Let $S$ be any completed trait of the pointed local Hurwitz model in
\cref{comp:hurwitz-pointed-23}.  On $S$, the normalization complex, equipped
with the two integral multiplicity vectors \(\mathcal C\) and \(T_D\), carries
a relative graded symmetric line
\(\mathfrak O_{\log}(\mathcal C,T_D)\) with the following properties.
\begin{enumerate}
\item It is functorial under simultaneous graph relabelling; under flat
base changes preserving the normalization and conductor diagrams with their
marked components, including
the finite splitting extensions used here; and under
gluing and semistable subdivision of the marked component tree.
\item On the generic fibre its \(\ZZ/2\)-degree is
\[
 \deg\mathfrak O_{\log,\eta}
   =Q(T_{D_y}+T_{D_z})=1+\epsilon(\Theta).
\]
\item The normalization--conductor filtration identifies its special-fibre
\(\ZZ/2\)-degree with
\[
 \deg\mathfrak O_{\log,s}
   =Q(P_D)+Q(T_D+T_D^n)=1+q(n)=1+e_{\mathrm{sing}}.
\]
The first summand comes from the node over the finite branch point, and the
second from the transition between the two marked wild branches.  All factors associated
with unmarked valence-two vertices occur in dual cohomological degrees and
cancel.
\end{enumerate}
\end{theorem}

\begin{proof}
Fix one such completed trait \(S\).  Over the \'etale locus of the
marked Galois cover, its self-fibre product is the disjoint union of the
deck-transformation graphs.  Its normalized graph algebra is therefore the
split based algebra
\[
 \mathcal A^\nu\simeq\prod_{g\in M_{23}}\mathcal O_{\Gamma_g}.
\]
The characteristic functions of \(T_{D_y}\), \(T_{D_z}\),
\(\mathcal C\), and \(T_D\) are integral idempotents in this
algebra.  Take the closures of the graph components, pull them back to the
finite scheme $B$ defined above, and only then push along the natural
degree-\(23\) quotient.  The resulting incidence algebra and its conductor
are finite over \(S\).  Additivity of this pushforward sends an integral
graph sum to its branchwise integral multiplicity vector.  Thus the generic
orbit idempotents and the special coefficients used below are the two
fibrewise restrictions of one normalization complex equipped with integral
multiplicities.  No identification by refined intersection theory is used
here.

After a splitting base change, the graph components give a finite free
coefficient system on the marked component tree joining the component over
the finite branch point to the two marked wild components.  Put
\[
 P_{D_y}=\mathcal C-T_{D_y},\qquad
 P_{D_z}=\mathcal C-T_{D_z}.
\]
These are idempotents corresponding to subsets because the normalizer orbits lie in
\(\mathcal C\), and
\[
 s(P_{D_y},P_{D_z})=s(T_{D_y},T_{D_z}).
\]
We retain the ordered pair \((P_{D_y},P_{D_z})\), rather than pass only to
the common symmetric-difference idempotent.  Each
complementary idempotent extends uniquely to the normalization of the finite
incidence algebra: an idempotent is integral, and the normalization
separates the generic graph components.  Their image modules are finite
projective over the splitting trait.

At the node over the finite branch point, the integral multiplicity data retain
\(\mathcal C\) and \(T_D\) separately, rather than only their common
reduction.  Concretely, the generic and special presentations of the
resulting graded symmetric line are
\[
\begin{aligned}
 \mathfrak O_{\eta}
   &=\mathfrak q(P_{D_y},P_{D_z})
     =\mathfrak q(T_{D_y},T_{D_z}),\\
 \mathfrak O_s
   &=\operatorname{qdet}(P_D)\otimes
     \mathfrak q(T_D,T_D^n)
     \quad\text{up to the polarization line.}
\end{aligned}
\]
These are the two fibrewise filtrations of one object in the relative
Grothendieck group, not standalone modules of the same rank.  Between the
marked charts, the identifications of bases on the annuli carry the
complementary idempotents.  They need
not descend separately as idempotents of the unnormalized graph algebra;
their conductor boundary is precisely the additional datum retained by
the normalization complex with multiplicities.

For a node \(e\), restrict these integral multiplicity vectors separately to
the two branches of the graph normalization and then push them to the sheet
correspondence.  Let \(B_{e,+}\) and \(B_{e,-}\) denote the resulting
integral based incidence coefficients, and use the same notation for their
associated virtual based modules.  The coefficient vector of their
difference is divisible by \(2\); no isomorphism between modules of unequal
ranks is asserted.  Hence
\[
 \mathcal D_e=B_{e,+}-B_{e,-}
\]
has even virtual rank and defines the node factor
\(\mathfrak a_e=\operatorname{qdet}(\mathcal D_e)\).

The ordered idempotents also specify which graded symmetric line is being
transported.  In the normalization--conductor filtration, expand the first
complementary idempotent as \(P_D=\mathcal C-T_{D_y}\) before cancelling the
two copies of \(\mathcal C\); the remaining finite term is \(P_D\).  The
second normalizer orbit is transported through the wild annuli and becomes \(T_D^n\)
at the other marked branch.  Thus the special presentation is the ordered
pair \((\mathcal C,T_D^n)\).  Since \(\mathcal C=P_D+T_D\) in the binary
permutation module, the line-valued polarization
\eqref{eq:qor-polarization} factors its degree into the factor defined by
\(P_D\) and the factor defined by \(T_D+T_D^n\).  The polarization factor
has degree
\[
 \langle P_D,T_D+T_D^n\rangle=0,
\]
whether \(q(n)=0\) or \(q(n)=1\).

Ferrand's conductor square supplies the branch restriction maps defining
the relative normalization complex \(\mathcal N^\bullet\) of the two image
modules with their integral multiplicity vectors.  Its generic fibre is the
complex with basis indexed by the selected graph components, and its
special fibre has the cellular presentation
\begin{equation}
 N_s^\bullet=
 \left[\bigoplus_{v}B_v\longrightarrow\bigoplus_eB_e\right],
\label{eq:effective-normalization-complex}
\end{equation}
with branch-difference differential and relative boundary conditions at the
marked vertices.  The hyperbolic completion
\[
 \mathcal N^\bullet\oplus
 R\!\operatorname{Hom}(\mathcal N^\bullet,R)[1]
\]
is \(1\)-shifted self-dual under the evaluation pairing.
The two ordered image modules in \(\mathcal N^\bullet\), with their integral
multiplicity vectors, form the chosen Lagrangian in this hyperbolic
completion.  Define \(\mathfrak O_{\log}\) to be its determinant line with
the induced symmetric form and grading.

The hypotheses and all the gluing assertions of
\cref{lem:effective-normalization-gluing} now apply.  It remains only to
identify the remaining local factors.  On the finite one-node chart
\(t=u^2\), the completed self-fibre product is
\[
 A=R[[u,v]]/\bigl((u-v)(u+v)\bigr),
\]
with normalization \(R[[u]]\oplus R[[u]]\).  The exact coefficient
sequence is
\[
 0\longrightarrow A\longrightarrow R[[u]]\oplus R[[u]]
 \xrightarrow{(f_+,f_-)\mapsto f_+(0)-f_-(0)}R
 \longrightarrow0.
\]
Thus its coefficient complex is the branch-difference complex
\[
 [\,B_{+}\oplus B_{-}\longrightarrow B_{\mathrm{cond}}\,].
\]
Restriction of an integral sum of graph components to the two normalized branches and
additive quotient pushforward commute, so applying this sequence to
\(\mathcal C\) and \(T_D\) separately gives
\(\mathcal D_{\mathrm{fin}}\) without a further comparison theorem.

On every wild annulus, Wewers's patching datum
\cite[Section~4.2]{Wewers} is a based isomorphism of
the graph-component modules with their integral multiplicities.  More precisely, one global patching datum is
\((\lambda;\varphi_j)\), with the same group isomorphism \(\lambda\) for
every tail.  Since a \(G\)-equivariant \(\varphi_j\) carries
\(\Gamma_g\) to \(\Gamma_{\lambda(g)}\), every internal annular
normalization cone is acyclic, with a contraction respecting the chosen
bases.  The only remaining factor on the wild
components is the difference between \(T_D\) and \(T_D^n\) at the second
marked branch.

The gluing lemma and its determinant filtration now identify the generic
remaining factor with the symmetric-difference module of \(T_{D_y}\) and
\(T_{D_z}\).  On the special fibre the remaining factors are the factor at
the node over the finite branch point, obtained from the divided
branch difference, and the symmetric-difference module of
\(T_D\) and \(T_D^n\).  Formula \eqref{eq:qor-polarization} combines their
degrees as the degree of the graded symmetric line attached to
\((\mathcal C,T_D^n)\); its polarization factor has degree
zero by the displayed cross-term calculation.  All unmarked transport
factors, gluing maps, and subdivisions are handled by
\cref{lem:effective-normalization-gluing}.

At the split node over the finite branch point, the exact branch ranks are
\[
 \operatorname{rank}B_+=1078,\qquad
 \operatorname{rank}B_-=112.
\]
Consequently
\[
 \deg\mathfrak a_{\mathrm{fin}}
   =\frac{1078-112}{2}=483\equiv1\pmod2.
\]
The complementary subset \(P_D=\mathcal C-T_D\) has
\(|P_D|/2=3542/2=1771\equiv1\pmod2\), the same parity.  More precisely,
the divided normalization coefficient consists of the eight simple-node
coordinates together with a residual class supported on the seven fixed
sheets.  The degree-three incidence relation in the Fano plane pairs this
residual class with seven of the eight node coordinates.  Modulo $2$, those
seven contributions cancel the corresponding coordinates, leaving the
coordinate of the distinguished node.  This is the remaining finite factor
in the gluing lemma.  At every
other internal node,
the common group isomorphism in Wewers's patching datum gives a based isomorphism
\(B_{e,+}\simeq B_{e,-}\), so \(\mathcal D_e=0\) as a virtual based module.
The difference between the two \(D\)-orbits under the comparison element
\(n\) is retained at the second marked branch, not as an additional factor
at an internal node.

Both generic normalizer orbits have size \(253\), so
\[
 \deg\mathfrak q(T_{D_y},T_{D_z})
 =253-|T_{D_y}\cap T_{D_z}|
 =1+\epsilon(\Theta).
\]
At the second marked branch the two orbits are equal when \(q(n)=0\).  When
\(q(n)=1\), they are disjoint, with \(T_D^n\subset2A(M_{23}^n)\).  Their
symmetric-difference ranks are \(0\) and \(506\), giving degrees \(0\) and
\(253\equiv1\).  This is \(q(n)\).  The filtered determinant identity now
gives the two fibre formulas in the statement.  Since the grading of a
relative graded symmetric line is locally constant on the trait, the two
displayed degrees are equal.

Finally, a change of splitting or of Wewers patching datum relabels every
graph by the same group isomorphism.  It is therefore an isometry of the
entire normalization complex with its integral multiplicities, so the line
descends from the splitting trait.  The remaining functorialities are
precisely those of
\cref{lem:effective-normalization-gluing}.
\end{proof}

\begin{corollary}[Comparison independent of branch-cycle certification]
\label{cor:quadratic-connector}
The line \(\mathfrak O_{\log}(\mathcal C,T_D)\) gives
\[
 1+\epsilon(\Theta)=1+e_{\mathrm{sing}},
\]
and hence a second proof of
\(\epsilon(\Theta)=e_{\mathrm{sing}}\) that does not use the certified
identification of the seven characteristic-zero branch cycles.
\end{corollary}

Ordinary nearby-cycle duality alone cannot produce this line.  It forgets
the chosen Lagrangian and identifies \(P_D\) with the congruent lift
\(P_D-2\mathcal C\); the latter has zero image modulo $4$ in the sheet
permutation representation.  By contrast, changing the integral lift by
\(-2\mathcal C\) changes the
\(\ZZ/2\)-degree by
\(|\mathcal C|=3795\equiv1\pmod2\).  Thus dependence on the two ordered
integral multiplicity vectors is necessary, not a choice of presentation.
No result of Lu--Zheng is used to identify the basiswise pairing with the product after
augmentation or to turn the divided branch-difference class into a
...
\section{Supporting arithmetic: the minimal-degree projection}
\label{sec:adjoints}

This section makes the distinguished cover explicit in the smallest
$\QQ$-defined source coordinate.  Its role in the main argument is
supporting rather than selective: the coordinate transports the Fano and
affine residue data exactly, but it does not distinguish one point of the
seven-cover Hurwitz scheme.
...
\section{Supporting arithmetic: the distinguished branch fibre}
...
what is supplied by the unpointed branch-fibre structures from the pointed
semistable data and integral multiplicities used in the relative determinant
calculation.
...
construction of \S\ref{sec:connector} may be formulated entirely in the
minimal-degree model.  The branch fibre by itself still does not distinguish
the degree-one Hurwitz point.  The determinant calculation also uses the two
marked wild components of the characteristic-$23$ stable map and the
integral multiplicity vectors on the graph components of the normalization
complex.
...
moduli problem.  The comparison in \cref{sec:connector} uses, in addition,
the marked chain of components in the semistable model and two integral
multiplicity vectors on the graph components; it is not a consequence of
the unpointed data.

We now leave the branch fibre for an independent global application of the
minimal equation.

\section{A separate application: arithmetic specializations}
...
The line in \cref{thm:quadratic-connector} is constructed from a based
normalization--conductor complex together with chosen integral multiplicity
vectors.  A natural foundational question is whether it is the image of a
canonical class in hermitian $K$-theory or in a Grothendieck--Witt category
of logarithmic correspondences, so that neither the bases nor the chosen
normalization presentation enter the definition.  Such a formulation would
not change the parity calculation, but could make the construction easier to
apply to other Hurwitz problems.

A separate arithmetic classification problem is to classify the
$2^4{:}\PSL_3(2)$-extensions of $\QQ$ unramified outside $\{2,3,23\}$ with
the local groups and constituent fields of \cref{thm:ramification}; in
particular, decide whether $M$ is unique.
...
formal annulus at the ramified $A_6$ node used in the relative determinant
calculation.
...
Finite and local ingredients for the relative determinant calculation &
...
\item We reconstruct the coordinate algebra of the inner Hurwitz scheme,
  ...
  and their branch cycles, and determine the $S_6$ Galois action on the
  degree-six component.
\item We determine the local decomposition and pointed reductions of that
  scheme at $23$, construct the idempotent defined by the singular-point
  coordinate directly from those reductions, determine the descent behavior of the
  finite-group invariants built from transporter elements, and compare their component idempotents in
  the exact reconstructed algebra.  We also construct a relative determinant
  line with its symmetric form and $\ZZ/2$-grading and obtain a second
  comparison by specialization, while proving that ordinary nearby cycles
  after forgetting the integral multiplicity data do not determine this
  line.
\item On the distinguished cover, we explicitly construct the degree-$4$
  function $W$ whose existence and minimality were established in
  \cite[Remark~3.4]{HJLPPZ}, its bidegree-$(23,4)$ equation, and exact
  certificates that it defines the same function field.  We determine the
  arithmetic of the fibre over $T=0$, including its Fano incidence, affine
  torsor, field tower, ramification, and obstruction theory, and transport
  this structure to the minimal coordinate.
...
theorem of \cite{HJLPPZ}.  The idempotent defined by the singular-point
coordinate gives an intrinsic characteristic-$23$ criterion separating the degree-one and
degree-six components of the reconstructed algebra.  The comparison theorem
proves its agreement with the augmentation of $\Theta$ by using the
exact branch-cycle identification of all seven reconstructed maps.  The
relative determinant line of
\cref{thm:quadratic-connector,cor:quadratic-connector} gives a second proof
from the pointed characteristic-$23$ geometry.  It is not an ordinary
nearby-cycle determinant: the nearby-cycle stalk has a basis indexed by the
individual sheets, and the coefficient Bockstein vanishes after the integral
multiplicity data are forgotten.
