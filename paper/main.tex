...
maps.  At $23$, the intrinsic position of the distinguished ADE singularity
has residual equation $(u-16)(u^2+1)(u^2+u+1)=0$ and directly constructs the
Boolean idempotent separating the degree-one and degree-six components.
Comparing this geometric idempotent intrinsically with the finite-group
...
\item \emph{Finite Hurwitz scheme.}  Exact reconstruction, elimination, and
  ...
  $1+6$ arithmetic decomposition of their moduli scheme; the
  characteristic-$23$ singular positions recover its component idempotent.
...
The direct reductions contain a little more arithmetic than their ADE
graphs alone.  If $P$ is one of the three closed points, let $\tau_P$ be
...
\begin{proposition}[Singular-position idempotent]
\label{prop:singular-position-idempotent}
Put $u(P)=t(\tau_P)$.  On the reduced special fibre of the normalized local
Hurwitz scheme, $u$ is a primitive coordinate with equation
\[
...
\[
\begin{array}{c|c|c}
\text{local point}&u(P)&\text{minimal polynomial over }\FF_{23}\\
\hline
\text{degree one, }E_8&16&u-16\\
\text{unramified degree }2,\ E_8&\sqrt{-1}&u^2+1\\
\text{ramified degree }4,\ A_6&r&r^2+r+1.
\end{array}
\]
...
of the coordinate algebra of the reduced special fibre.  The three factors
...
Thus the characteristic-$23$ models themselves construct the Boolean
morphism cutting out the degree-six component; no complex Nielsen labels or
continued branch cycles enter \cref{prop:singular-position-idempotent}.
What remains open is the conceptual comparison
\[
 \epsilon(\Theta)=e_{\mathrm{sing}}.
\]
The exact branch-cycle certificate proves this equality indirectly because
both sides are $e_6$, but it does not construct the relative-transporter
augmentation from the stable geometry.  Resolving only the coarse ADE
source is insufficient: the degree-one and unramified quadratic reductions
both have geometric type $E_8$.  A proof must retain the arithmetic
singular position together with the incidence and gluing in a common
pointed $M_{23}$-frame.
...
\item Compare the intrinsic singular-position idempotent
  $e_{\mathrm{sing}}$ of \cref{prop:singular-position-idempotent} with the
  relative-transporter augmentation $\epsilon(\Theta)$ without first using
  the exact branch cycles.  The characteristic-$23$ models already construct
  a Galois-compatible Boolean morphism cutting out the degree-six component,
  while the richer two-endpoint and cyclic-conjugacy data cannot themselves
  descend, by \cref{prop:relative-transporter-descent}.  More concretely,
  resolve the $E_8$ and $A_2+A_6$ deformations of
  \cref{comp:hurwitz-pointed-23} as pointed stable $M_{23}$-maps and construct
  an incidence-weighted binary cycle whose augmentation is
  $e_{\mathrm{sing}}(P)=f(P)-1$.
\end{enumerate}
...
\item We reconstruct the coordinate algebra of the inner Hurwitz scheme,
  ...
  $23$, construct the singular-position idempotent directly from those
  reductions, and determine the descent behavior of the
  relative-transporter invariants.
...
theorem of \cite{HJLPPZ}.  The singular-position idempotent gives an
intrinsic characteristic-$23$ criterion separating the degree-one and
degree-six components of the reconstructed algebra.  The local
residue-degree parity agrees with the binary augmentation, but we do not
construct that agreement intrinsically from pointed stable $M_{23}$-maps.
Resolving the $E_8$ and $A_2+A_6$ deformations in a common Mathieu frame and
identifying the resulting incidence cycle with the singular-position
idempotent remain open.
