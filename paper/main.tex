...
We give a geometric characterization of its distinguished point.
On each genus-$4$ quotient curve, take the osculating planes at the
two totally ramified points.  Their intersection meets the curve in
two reduced points for exactly one of the seven covers and is disjoint
from the other six.  This condition is preserved by Galois conjugation
and by exchange of the marked points, so its uniqueness gives a
model-based proof of the distinguished point's global Galois fixedness.
Equivalently, only that curve has points $C,D$ with
$[C-D]=3[A-B]$, where $A,B$ are the totally ramified points.
Its osculating pencil defines a degree-$4$ map with geometric monodromy
$S_4$.
The incidence calculation is exact over the full Hurwitz algebra
$K_0\times L$, where $K_0=\QQ(\sqrt{-23})$, $[L:K_0]=6$, and the
relative Galois closure has group $S_6$.
Exact equations and certified branch cycles identify all seven covers.
We also determine their pointed reductions at $23$ and compare the
resulting component idempotent with a finite-group invariant.
A specialization theorem forcing the global osculating incidence
remains open.
For the distinguished cover we construct a separate minimal
bidegree-$(23,4)$ equation, determine the Fano-plane and affine
$\FF_2^3$-torsor arithmetic of its involution branch fibre, and give
a congruence class of specializations with Galois group $M_{23}$ and
controlled ramification.  Exact data and verification scripts accompany
the results.
\end{abstract}
...
The construction uses the curve and the unordered pair of marked points.
...
characterization and a computational proof of fixedness, not an
equation-free proof that the branch-cycle data force the incidence.
We keep this distinction throughout.
...
A geometric argument forcing the global incidence from the reduction
at $23$ is the principal remaining question.
...
pointed reductions at $23$.  The curve-theoretic argument uses ordinary
canonical linear series and Riemann--Roch; the particular $2$ versus $0$
incidence is an exact calculation on this family.
...
\Cref{sec:reduction} studies reduction and compares the three
idempotents, including the limitations of a cohomological approach.
...
\section{Reduction at \texorpdfstring{$23$}{23} and comparison of the components}
\label{sec:reduction}
...
We have not proved that the harmonic condition forces the
characteristic-zero incidence.
...
\subsection*{1. Existence of the osculating incidence}
Can one show, without reconstructing all seven maps, that
\[
 \mathcal Z=\mathcal X\cap H_A\cap H_B
\]
is nonempty and supported over one geometric Hurwitz point?
Equivalently, why does $3[A-B]$ lie on the difference variety $X-X$
for exactly one member of this Nielsen class?
The present proof establishes this by exact equations.
A global intersection calculation would give a different source
of the required existence and uniqueness.

\subsection*{2. Relation with reduction at $23$}
Can the harmonic position in \cref{prop:harmonic-position} force
the osculating incidence in characteristic zero?
The equality of their component idempotents is already proved,
but not by specialization of $\mathcal Z$.
One would need to control the extensions of the canonical jet kernels
and their common zero scheme through the degeneration.
A closed-fibre common zero need not lift; proving an appropriate
flatness or lifting statement is essential.
The bad-reduction theory of three-point covers \cite{Wewers} provides
a framework, not this missing implication.
...
do not certify the relative geometric construction left open in
\cref{sec:open}.
...
satisfies it.  Its existence proof is therefore computational.
We do not claim that the branch-cycle classes alone force that
incidence, or that its existence follows from a local rank-one
factor or from the harmonic reduction.
...
No relative quadratic determinant line or second specialization
proof is asserted.
...
in \cref{sec:certificates}; these checks do not certify the unresolved
relative geometric construction.
...
\emph{The Stacks Project}, Tags 04GK and 09XD,
